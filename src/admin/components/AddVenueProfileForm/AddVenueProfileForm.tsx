...
const AddVenueProfileForm = () => {
    ...
    const [venueBio,setVenueBio]=useState<any>({value:"",showError:false,error:"Venue bio is required"});
    //Ref
    ...
    const getTags = async () => {
        try {
            const response = await axios.get("/v1/filter", {
    ...
    const onSubmit = async (e: any) => {
        if((venueBio.value).length<1){
            setVenueBio({showError:true,value:"",error:"Venue bio is required"});
            return;
        }
        setLoading(true);
                            ...
                            {form.errors.coverPhoto}
                                    ...
                                    <div>
                                        ...
                                        <textarea
                                            ...
                                            name="venueBio"
                                            value={venueBio.value}
                                            onChange={(e) => {
                                                if((e.target.value).length>0){
                                                    setVenueBio({value:e.target.value,showError:false,error:venueBio.error});
                                                }else{
                                                    setVenueBio({value:e.target.value})
                                                }
                                            }}
                                        ></textarea>
                                        {venueBio.showError ? (
                                            <TextFieldErrorStyle>{venueBio.error}</TextFieldErrorStyle>
                                        ) : null}
                                    </div>
